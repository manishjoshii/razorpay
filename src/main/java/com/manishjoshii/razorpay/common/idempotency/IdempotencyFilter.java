package com.manishjoshii.razorpay.common.idempotency;
...
public class IdempotencyFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        if (rawKey == null || rawKey.isBlank()) {   // No Idempotency key found, continue
            chain.doFilter(request, response);
            return;
        }
        ...
        if (!claimed) {
            // another thread has claimed this key
        ...
        try {
            ...
            if (status < 400 && bodyBytes.length > 0) {
                ...
                // Error or empty - delete placeholder so client can retry cleanlly
